...
ExecStart=/usr/bin/python -m uvicorn vood.server.playwright.render_server:app --host localhost --port 4000
Restart=always
RestartSec=5
...
<dict>
    ...
    <array>
        ...
        <string>vood.server.playwright.render_server:app</string>
        <string>--host</string>
...
```cmd
nssm install VoodPlaywrightServer "C:\Python311\python.exe" "-m uvicorn vood.server.playwright.render_server:app --host localhost --port 4000"
...
```bash
python -m uvicorn vood.server.playwright.render_server:app --host localhost --port 4000
```
...
# Run server
CMD ["uvicorn", "vood.server.playwright.render_server:app", "--host", "0.0.0.0", "--port", "4000"]
```
