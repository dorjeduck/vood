...
def _default_path() -> SVGPath:
    """Factory function for default path"""
    return paths.line(0, 0, 100, 0)
...
class PathTextState(TextState):
    ...
    path: SVGPath = field(
        default_factory=_default_path
    )  # The path to follow (supports morphing!)
    offset: float = 0.0  # Position along path (0.0 to 1.0)
