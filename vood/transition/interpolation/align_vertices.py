...
def _get_hole_mapper_from_config() -> HoleMapper:
    ...
    """
    config = get_config()
    strategy = config.get("morphing.hole_mapper", "clustering")

    if strategy == "greedy":
        return GreedyNearestMapper()
    elif strategy == "clustering":
        balance = config.get("morphing.clustering.balance_clusters", True)
        max_iter = config.get("morphing.clustering.max_iterations", 50)
        seed = config.get("morphing.clustering.random_seed", 42)
