...
class ClusteringMapper(HoleMapper):
    ...
    def map(
        self, holes1: List[VertexLoop], holes2: List[VertexLoop]
    ...
    def _match_with_clustering(
        self, holes1: List[VertexLoop], holes2: List[VertexLoop], cluster_sources: bool
    ) -> Tuple[List[VertexLoop], List[VertexLoop]]:
    ...
    def _match_with_merging(
        ...
        for cluster_idx in range(k):
            cluster_holes = [
                holes1[i] for i, c in enumerate(clusters) if c == cluster_idx
            ]
            if cluster_holes:
        ...
        for cluster_idx, cluster_centroid in cluster_centroids:
            ...
            cluster_holes = cluster_hole_lists[
    ...
    def _match_with_splitting_DISABLED(
        ...
        for src_cluster_idx, dst_cluster_idx in cluster_pairs:
            ...
            src_cluster_holes = [
                holes1[i] for i, c in enumerate(src_clusters) if c == src_cluster_idx
            ]
            dst_cluster_holes = [
                holes2[i] for i, c in enumerate(dst_clusters) if c == dst_cluster_idx
            ]

            # Map holes within this cluster pair
            paired1, paired2 = self._map_within_cluster_pair(
                src_cluster_holes, dst_cluster_holes
            )
    ...
    def _map_within_cluster_pair(
        self, src_holes: List[VertexLoop], dst_holes: List[VertexLoop]
    ) -> Tuple[List[VertexLoop], List[VertexLoop]]:
