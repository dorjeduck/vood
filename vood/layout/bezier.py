...
def bezier(
    ...
    def bezier_tangent_angle(t, pts):
        ...
        dx = sum(
            n * math.comb(n - 1, i) * (1 - t) ** (n - 1 - i) * t**i * (pts[i + 1][0] - pts[i][0])
            for i in range(n)
        )
        dy = sum(
            n * math.comb(n - 1, i) * (1 - t) ** (n - 1 - i) * t**i * (pts[i + 1][1] - pts[i][1])
            for i in range(n)
        )
        ...
        if dx == 0 and dy == 0:
            ...
            if t < 1 - dt:
                ...
                x2, y2 = bezier_point(t + dt, pts)
            else:
                x1, y1 = bezier_point(t - dt, pts)
                x2, y2 = bezier_point(t, pts)
            dx, dy = x2 - x1, y2 - y1
        
        angle = math.degrees(math.atan2(dy, dx))
        return angle

    def build_arc_length_table(pts, samples=1000):
